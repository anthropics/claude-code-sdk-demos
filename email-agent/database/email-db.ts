...
export class EmailDatabase {
  ...
  private initializeDatabase(): void {
    ...
    let emailColumns = this.db
      .prepare('PRAGMA table_info(emails)')
      .all() as Array<{ name: string }>
    let hasImapUid = emailColumns.some((column) => column.name === 'imap_uid')

    if (!hasImapUid) {
      this.db.exec('ALTER TABLE emails ADD COLUMN imap_uid INTEGER')
      emailColumns = this.db
        .prepare('PRAGMA table_info(emails)')
        .all() as Array<{ name: string }>
      hasImapUid = emailColumns.some((column) => column.name === 'imap_uid')
    }
  ...
  insertEmail(
    ...
    const insertTransaction = this.db.transaction(() => {
      const result = insertEmail.run({
        ...
        $date_sent:
          typeof email.date_sent === 'string'
            ? email.date_sent
            : email.date_sent.toISOString(),
        $date_received: email.date_received
          ? typeof email.date_received === 'string'
            ? email.date_received
            : email.date_received.toISOString()
